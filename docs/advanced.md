...
### 🧠 深度智能与上下文
*   **历史回溯 (Context)**：可选在群聊中获取最近 N 条历史消息（默认 10），用于需要“强制保留上下文原文”的场景。
...
| `historyLimit` | number | `10` | **历史消息条数**。默认会在每次触发时附带最近群内原文；如需仅依赖 OpenClaw 会话系统可设为 `0`。 |
...
> 当前默认 `historyLimit = 10`，开箱即用保留邻近群聊上下文。
> 若你更关注 token 成本/噪音，可改为 `historyLimit = 0`；若想缩短窗口可设为 `3~5`。
>
...
A: 这是 `historyLimit` 的行为。当前版本默认 `10`，即会附加最近群内原文消息。
如果你把 `historyLimit` 设为 `0`，则不额外拼接群历史，主要依赖 OpenClaw 会话系统管理上下文（更接近 Telegram 行为）。
...
*   调整默认上下文策略：`historyLimit` 默认值为 `10`，会默认注入附近群历史原文。
